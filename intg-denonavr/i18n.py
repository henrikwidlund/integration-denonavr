...
AVAILABLE_LANGUAGES = ["en_US", "de_DE", "es_ES", "fr_FR", "it_IT", "sv_SE"]
...
def get_translator(language: str | None = None) -> NullTranslations:
    ...
    if lang not in _translators:
        try:
            _translators[lang] = translation(
                "intg-denonavr", localedir=str(LOCALE_DIR), languages=[lang], fallback=True
            )
